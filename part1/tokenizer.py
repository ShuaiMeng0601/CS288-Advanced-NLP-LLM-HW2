...
class Tokenizer:
    ...
    def _bpe(self, token_bytes: bytes) -> list[bytes]:
        ...
        while True:
            ...
            pairs = self._get_pairs(tokens)  # Get initial pairs
            if not pairs:
                break
            ...
            for pair in pairs:
                ...
                if merged in self.inverse_vocab:
                    rank = self.inverse_vocab[merged]  # Get token ID as rank
                else:
                    rank = float('inf')  # Not in vocab, can't merge
    ...
    def _encode_chunk(self, text: str) -> list[int]:
        ...
        for match in self.pat.finditer(text):
            ...
            byte_seq = self._bpe(bytes(pre_token, "utf-8"))
            for seq in byte_seq:
                if seq in self.inverse_vocab:
                    ids.append(self.inverse_vocab[seq])
                else:
                    # Handle unknown token by encoding individual bytes
                    for b in seq:
