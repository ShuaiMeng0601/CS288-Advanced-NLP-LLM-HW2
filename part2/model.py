...
class Embedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, d_model: int):
        ...
        # nn.Parameter 一般比nn.Linear 好的地方在于前者可以进行weight tying这种操作
        self._init_weights()
...
def softmax(x: Tensor, dim: int = -1) -> Tensor:
    ...
    #为了实现数值稳定，需要利用 Softmax 的平移不变性，在计算指数之前先减去输入张量在目标维度上的最大值
    # 这样可以防止指数函数的输入过大导致溢出，同时不改变结果的相对大小。
    # x_{\text{safe}} = x - \max(x)
    x_safe = x - torch.max(x, dim=dim, keepdim=True)[0]
    sum = torch.sum(torch.exp(x_safe), dim=dim, keepdim=True)
    result = torch.exp(x_safe) / sum
    # 全部被 mask 时 sum 为 0，产生 NaN，替换为 0
    result = result.nan_to_num(0.0) #考虑这一种情况：当所有位置都被 mask 掉时，softmax 的输入是全 -inf，这会导致 exp(-inf) = 0，sum 也是 0，从而产生 NaN。我们可以使用 torch.nan_to_num 将 NaN 替换为 0.0，这样在这种极端情况下 softmax 输出将是全 0 的分布。
    return result
...
def silu(x: Tensor) -> Tensor:
    ...
    return x / (1 + torch.exp(-x)) # SiLU(x) = x * sigmoid(x) = x / (1 + exp(-x))
...
class RotaryPositionEmbedding(nn.Module):
    ...
    def __init__(self, d_model: int, max_seq_len: int, theta: float = 10000.0):
        ...
        # TODO: Implement inv_freq 1 / (base ^ (2i / d))
        inv_freq = 1.0 / (theta ** (2* torch.arange(0, d_model//2).float() / d_model))
    ...
    def forward(self, x: Tensor, token_positions: Tensor) -> Tensor:
        ...
        if x.dim() == 4:
            ...
            # cos/sin shape: (batch, seq_len, d_k) -> unsqueeze(1) to (batch, 1, seq_len, d_k)
            cos = cos.unsqueeze(1)
            sin = sin.unsqueeze(1)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        attn = scaled_dot_product_attention(Q, K, V, self._create_causal_mask(seq_len=seq_len, device=x.device))
        # attn size is (batch, num_heads, seq_len, d_k)
        # we need it to be (batch, seq_len, d_model) for the output projection
...
class MultiHeadSelfAttentionWithRoPE(nn.Module):
    ...
    def forward(self, x: Tensor, token_positions: Optional[Tensor] = None) -> Tensor:
        ...
        # RoPE加的位置要注意，是加在Q和K上，而不是V上 而且是在算attention之前加的
        # 而且我们只需要apply RoPE 在Q和K上就行了，不需要加位置编码
        
        Q = self.q_proj(x).view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1,2)
        K = self.k_proj(x).view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1,2)
        V = self.v_proj(x).view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1,2)

        #rope = RotaryPositionEmbedding(self.d_k, seq_len, self.theta)
        #rope = rope.to(x.device)
...
class TransformerLM(nn.Module):
    ...
    def forward(self, token_ids: Tensor, token_positions: Optional[Tensor] = None) -> Tensor:
        ...
        for layer in self.layers:
            x = layer(x, token_positions) #每一层都要传入token_positions 因为每一层的attention都要用到RoPE，RoPE的input有一个就是token_positions
        x = self.final_ln(x)
        x = self.output(x)
...
def count_flops_per_token(
    ...
    # Embedding lookup: negligible FLOPs (just indexing)
    
    # Per layer:
    for _ in range(num_layers):
        # Q, K, V projections: each is (1, d_model) x (d_model, d_model) = 2 * d_model^2
        flops += 3 * 2 * d_model * d_model
        
        # Attention: Q @ K^T: per token, attending to context_length keys = 2 * d_model * context_length
        flops += 2 * d_model * context_length
        
        # Attention: weights @ V: 2 * context_length * d_model
        ...
        # SwiGLU FFN:
        # w1: d_model -> d_ff: 2 * d_model * d_ff
        flops += 2 * d_model * d_ff
        # w3: d_model -> d_ff: 2 * d_model * d_ff
        flops += 2 * d_model * d_ff
        # w2: d_ff -> d_model: 2 * d_ff * d_model
